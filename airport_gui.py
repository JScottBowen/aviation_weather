...
def get_taf_str(icao):
    ''' (str)-> str

    Return an airport's unencoded meteorological aviation routine (METAR) report
    retrieved from the NOAA government ftp server in string format.
...
def get_latest_report(icao):
    ...
    taf_str = get_taf_str(icao)
    shorttaf_str = get_shorttaf_str(icao)
...
def calculate(*args):
    try:
        value = icao.get()
        wx_data.set(process_line(get_latest_report(value)))
    except ValueError:
        pass

root = Tk()
root.title("Get latest Forecast")
...
icao_entry = ttk.Entry(mainframe, width=5, textvariable=icao)
icao_entry.grid(column=2, row=1, sticky=W)

ttk.Label(mainframe, textvariable=wx_data).grid(column=2, row=2, sticky=(W, E))
ttk.Button(mainframe, text="Get TAF", command=calculate).grid(column=3, row=1, sticky=E)

ttk.Label(mainframe, text="ICAO Identifier:").grid(column=1, row=1, sticky=W)
ttk.Label(mainframe, text="weather data:").grid(column=1, row=2, sticky=E)
